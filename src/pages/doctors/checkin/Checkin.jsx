...
const TABLE_HEADERS = ['№', 'Bemorning ismi', 'Yoshi', 'Tashrif', 'Tibbiy ma\'lumot', 'Tarix', 'Amal'];
...
const Checkin = () => {
    ...
    const { data, isLoading, isError, error, refetch, isFetching } = useGetStoriesByDoctorQuery(workerId, {
        skip: !workerId,
    });
    ...
    const handleConsultPatient = useCallback((patient) => {
    ...
    const validateForm = useCallback(() => {
        const errors = {};
        if (!formData.height || formData.height <= 0) errors.height = 'Bo\'y kiriting (musbat son)';
        if (!formData.weight || formData.weight <= 0) errors.weight = 'Vazn kiriting (musbat son)';
    ...
    const handleSaveMedicalInfo = useCallback(async () => {
        ...
        try {
            ...
            await updateBmiPotsents({ id: selectedPatient?.patientId?._id, data }).unwrap();
            ...
            toast.error(`Xatolik yuz berdi: ${err?.data?.message || 'Ma\'lumotlarni saqlashda xato'}`, {
                position: 'top-right',
                autoClose: 5000,
            });
        }
    ...
    const sortedPatients = useMemo(() => {
        return data?.innerData?.patients?.slice().sort((a, b) => a.order_number - b.order_number) || [];
    }, [data]);
    ...
    if (isError) {
        return (
            <div className="doctor-appointment-system error-container">
                ...
                <p>Xatolik yuz berdi: {error?.data?.message || error?.message || 'Maʼlumotlarni yuklashda xato'}</p>
                <button
                    onClick={() => {
                        refetch();
                    ...
                    <div className="queues-header">
                        <div className="header-info">
                            ...
                            <div>
                                ...
                                <p>
                                    Bugungi navbat:{' '}
                                    {(data?.innerData?.todayUnviewedCount + data?.innerData?.todayViewedCount) || 0} bemor
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            {sortedPatients.map((patient) => (
                                <tr key={patient._id} className={`patient-row ${patient.view ? 'completed' : ''}`}>
                                    ...
                                    <td className="medical-info">
                                        {patient?.patientId?.height && patient?.patientId?.weight && patient?.patientId?.bmi && patient?.patientId?.bloodGroup ? (
                                            <div className="medical-info-container">
                                                <div className="bmi-info">
                                                    <Scale size={12} color="#6b7280" aria-hidden="true" />
                                                    <span
                                                        className={`bmi-value bmi-${getBMIStatus(patient?.patientId?.bmi)}`}
                                                        style={{ color: getBMIColor(patient?.patientId?.bmi) }}
                                                    >
                                                ...
                                                <div className="blood-group-info">
                                                    ...
                                                    <span className="blood-group-value">{patient?.patientId?.bloodGroup}</span>
                                                </div>
                                                <div className="physical-stats">
                                                    {patient?.patientId?.height}cm, {patient?.patientId?.weight}kg
                                                </div>
                                            </div>
                                        ) : (
                                            <button
                    ...
                    <div className="modal-content">
                        ...
                        <div className="modal-patient-info">
                            <p className="patient-info-text">
                                <strong>{selectedPatient?.patientId.name}</strong> uchun tibbiy ma'lumot
                            </p>
                        </div>
