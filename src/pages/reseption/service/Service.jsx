...
import {
    FaStethoscope, FaUserMd, FaIdBadge, FaNotesMedical, FaCoins, FaPlus, FaMinus,
    FaSave, FaTimes, FaListAlt, FaEdit, FaTrash, FaChevronDown, FaChevronUp,
    FaHospital, FaHeartbeat, FaCapsules,
} from 'react-icons/fa';
import { MdMedicalServices, MdLocalHospital, MdHealthAndSafety } from 'react-icons/md';
import { RiMedicineBottleFill, RiStethoscopeFill } from 'react-icons/ri';
import { Select } from 'antd';
import {
    useGetServicesQuery, useCreateServiceMutation, useUpdateServiceMutation,
    useDeleteServiceMutation, useAddServiceItemMutation, useDeleteServiceItemMutation,
...
const ServiceItem = memo(({ index, service, handleServiceChange, removeServiceField, showRemove }) => (
...
function Service() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.profession || !formData.doctorId) return;
        console.log(formData);

        const serviceData = {
            profession: formData?.profession,
            doctorId: formData?.doctorId,
            services: formData?.services?.filter((s) => s.name && s.price)?.map((s, idx) => ({
                // Generate a temporary ID for new services
                _id: editingService ? s._id || `new_${Date.now()}_${idx}` : `new_${Date.now()}_${idx}`,
                name: s.name,
        ...
        try {
            if (editingService) {
                await updateService({ id: editingService._id, ...serviceData }).unwrap();
                setEditingService(null);
            } else {
                await createService(serviceData).unwrap();
            }
            setFormData({ profession: '', doctorId: '', services: [{ name: '', price: '' }] });
        } catch (error) {
    ...
    const handleEdit = (service) => {
        ...
        setFormData({
            ...
            services: service.services.length
                ? service.services.map((s) => ({ name: s.name, price: s.price.toString() }))
                : [{ name: '', price: '' }],
        });
    };
    ...
    // Modified to use service name for deletion since _id is not available
    const handleDeleteServiceItem = async (serviceId, itemName) => {
        if (window.confirm("Ushbu xizmat elementini o'chirishni xohlaysizmi?")) {
            try {
                // Pass the serviceId and itemName to the deleteServiceItem mutation
            ...
            <div className="form-container">
                ...
                <form onSubmit={handleSubmit} className="form">
                        ...
                        <div>
                            ...
                            <Select
                                ...
                                options={workers?.innerData
                                    ...
                                    .map((worker) => ({
                                        label: `${worker.firstName} ${worker.lastName}: ${worker.specialization}`,
                                        value: worker._id,
                                    }))}
                            />
                        </div>
                    ...
                    <div ref={scrollContainerRef} className="services-section">
                        ...
                        <button type="button" onClick={addServiceField} className="add-service-button">
                            <FaPlus />
                            Xizmat Qo'shish
                        </button>
                    </div>
                    ...
                    <div className="form-buttons">
                        <button type="submit" className="submit-button" disabled={isCreating || isUpdating}>
                            <FaSave />
                ...
                <div className="scrollable-table">
                    {isLoading ? (
                        ...
                        <table className="table">
                            ...
                            <tbody>
                                {Array.isArray(services?.innerData) && services.innerData.length > 0 ? (
                                    services.innerData.map((service, serviceIndex) => (
                                        <React.Fragment key={service._id}>
                                            <tr className={serviceIndex % 2 === 0 ? 'even-row' : 'odd-row'}>
                                                <td>
                                                            ...
                                                            <table className="expanded-table">
                                                                <thead>
                                                                    <tr>
                                                                        <th>
                                                                            <div>
                                                                                <RiMedicineBottleFill style={{ color: '#10b981' }} />
                                                                                Xizmat Nomi
                                                                            </div>
                                                                        </th>
                                                                        <th>
                                                                ...
                                                                <tbody>
                                                                    {Array.isArray(service.services) && service.services.length > 0 ? (
                                                                        service.services.map((s, itemIndex) => (
                                                                            <tr
                                                                                key={itemIndex}
                                                                                className={itemIndex % 2 === 0 ? 'even-row' : 'odd-row'}
                                                                            >
                                                                                <td>
                                                                                ...
                                                                                <td>
                                                                                    <button
                                                                                        onClick={() => handleDeleteServiceItem(service._id, s.name)}
                                                                        ...
                                                                        <tr>
                                                                            <td colSpan="3" className="empty-services-message">
                                                                                <div className="empty-services-content">
                                                                                    <FaStethoscope size={24} style={{ color: '#9ca3af' }} />
                                    ...
                                    <tr>
                                        <td colSpan="4" className="empty-table-message">
                                            <div className="empty-table-content">
                                                <MdLocalHospital size={32} style={{ color: '#9ca3af' }} />
                                                <div>
